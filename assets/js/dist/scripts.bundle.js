    ...
    */
		...
		_proto.init = function init4(vars) {
			...
			var id,
				...
				_onClick = function _onClick2(e) {
					return (
						_ignoreCheck(e) || (_getEvent(e, preventDefault) && onClick2(self))
					);
				};
	...
	function updateSlides() {
		...
		if (params.setWrapperSize) {
			wrapperEl.style[getDirectionLabel('width')] = ''.concat(
				swiper.virtualSize + spaceBetween,
				'px'
			);
		}
	...
	function onTouchEnd(event2) {
		...
		if (
			...
			const proceed =
				['pointercancel', 'contextmenu'].includes(event2.type) &&
	...
	function Zoom(_ref) {
		...
		function eventWithinZoomContainer(e) {
			...
			if (
				[...swiper.hostEl.querySelectorAll(selector3)].filter((containerEl) =>
	...
	function Grid(_ref) {
		...
		const updateWrapperSize = (slideSize, snapGrid, getDirectionLabel) => {
			...
			swiper.wrapperEl.style[getDirectionLabel('width')] = ''.concat(
				swiper.virtualSize + spaceBetween,
				'px'
			);
			if (centeredSlides) {
	...
	if (sidebarProduct) {
		...
		const filterWrapperGroup =
			sidebarProduct.querySelectorAll('.filter-wrapper');
		...
		if (filterChips) {
			filterChips.forEach((el) => {
				el.addEventListener('click', (e) => {
					...
					const checkInp = document.querySelector(
						"input[name='".concat(dataFilter, "']")
					);
